-- Mobile-Optimized Database Schema and Configuration - FIXED VERSION
-- Optimized for Supabase SQL Editor (without CONCURRENTLY)
...
-- FIXED: Removed CONCURRENTLY for Supabase compatibility
...
ALTER TABLE casinos ADD COLUMN IF NOT EXISTS search_vector tsvector;
...
CREATE POLICY IF NOT EXISTS "Mobile public read access to active casinos"
...
CREATE POLICY IF NOT EXISTS "Users can submit and manage their own reviews"
...
BEGIN
  ...
  result := json_build_object(
    ...
    'totalCount', (SELECT COUNT(*) FROM mobile_casino_dashboard)
  );
...
GRANT SELECT ON mobile_casino_dashboard TO public;

-- Create maintenance functions
CREATE OR REPLACE FUNCTION cleanup_mobile_indexes()
RETURNS TEXT AS $$
BEGIN
  -- Reindex mobile-optimized indexes (without CONCURRENTLY)
...
BEGIN
  -- Analyze search function performance (mock data for now)
