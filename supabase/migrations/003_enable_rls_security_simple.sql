-- Enable Row Level Security (RLS) and create security policies - SIMPLE VERSION
-- This migration will secure sensitive tables from unauthorized access
-- Simplified approach to avoid syntax errors
...
-- Step 7: Grant execute permission on admin check function
GRANT EXECUTE ON FUNCTION is_user_admin(UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION is_user_admin() TO authenticated;

-- Step 8: Create secure admin access policy function
...
-- Step 9: Update admin_users policy to use the new function
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'admin_users') THEN
        DROP POLICY IF EXISTS "Only super admins can access admin_users" ON admin_users;
